...
if [ -d "arm32-gcc" ]; then
	...
	time git clone https://bitbucket.org/UBERTC/arm-linux-androideabi-4.9 -b master --depth 1 arm32-gcc
fi
if [ -d "arm64-gcc" ]; then
...
pack() {
VERSION="$(date +'%Y%m%d-%H%M')-g$(git rev-parse --verify --short=8 HEAD 2>/dev/null)"
